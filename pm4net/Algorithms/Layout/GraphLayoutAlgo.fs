...
module internal GraphLayoutAlgo =
    ...
    let private insertSequence (rankGraph: GlobalRankGraph) components sequence =
        ...
        let rec traverse (rankGraph: GlobalRankGraph) numRanks node visited =
    ...
    let internal computeGlobalRankingAndSkeletonForVariations variations =
        ...
        let updateDuplicateEdges (rankGraph: GlobalRankGraph) variation =
    ...
    let internal fixHorizontalEdgesInGlobalRankGraphForDiscoveredModel (rankGraph: GlobalRankGraph) components (model: DirectedGraph<Graphs.Node, Graphs.Edge>) =
    ...
    let internal constructDiscoveredGraph mergeEdges (goNsg: GlobalOrderNodeSequenceGraph) (skeleton: Skeleton) (model: DirectedGraph<Graphs.Node, Graphs.Edge>) =
        ...
        let updateNodeAndReferencedEdges (graph: DiscoveredGraph) originalNode newNode =
            { graph with
                ...
                Edges = graph.Edges |> List.map (fun (a, b, freq) ->
                    if a = originalNode then (newNode, b, freq)
                    else if b = originalNode then (a, newNode, freq)
                    else (a, b, freq)) }
        ...
        let closeGaps (graph: DiscoveredGraph) =
            ...
            let moveIfPossible (graph: DiscoveredGraph) (nodes: GraphNode list) left rank =
                (graph, nodes) ||> List.fold (fun graph node ->
                    ...
                    | true -> graph // Too close to the backbone, do not move onto the backbone or over it
                    | false ->

                        // Find the closest existing node on the same rank and the same side of the backbone, that are also closer to the backbone than the current node
                        let closestExisting =
                            graph.Nodes
                            |> List.filter (fun n ->
                                let tmpPos = getPosition n
                                tmpPos.Y = rank && if left then tmpPos.X < 0f && tmpPos.X > pos.X else tmpPos.X > 0f && tmpPos.X < pos.X)

                        // Check whether there is an existing node closest to current node
                        ...
                        | _ ->
                            ...
                            let proposedXPos =
                                ...
                                | ConstrainedVirtual _, ConstrainedVirtual _ -> if left then (getPosition closest).X - 1f else (getPosition closest).X + 1f
                                | UnconstrainedVirtual _, UnconstrainedVirtual _ -> (getPosition closest).X
                                | _ -> if left then (getPosition closest).X - 0.5f else (getPosition closest).X + 0.5f

                            match proposedXPos = pos.X with
                            | true -> graph // Proposed position is already the current position, so there is nothing else to do
                            | _ ->
                                match node with
                                | ConstrainedReal _
        ...
        let crossingMinimisation (graph: DiscoveredGraph) =
            ...
            let countEdgeCrossings (graph: DiscoveredGraph) =
                ...
                graph.Edges |> List.sumBy (fun (a, b, _) ->
                    let posA, posB = getPosition a, getPosition b
                    graph.Edges
                    |> List.filter (fun (c, d, _) ->
            ...
            let wmedian topDown (graph: DiscoveredGraph) =
                ...
                let medianValue node adjRank (graph: DiscoveredGraph) =
                    let p =
                        graph.Nodes
                        |> List.filter (fun n -> getYPosition n = adjRank && graph.Edges |> List.exists (fun (a, b, _) -> a = node && b = n))
    ...
    let internal computeNodePositions maxCharsPerLine nodesep ranksep edgesep (graph: DiscoveredGraph) =
        ...
        // Add all edges. Direct ones from real to real nodes need to have a Connection object created, those connecting to a virtual node already have some
        let edges =
            graph.Edges
            |> List.map (fun (a, b, freq) ->
                match a, b with
                | ConstrainedReal(_, _, aName), ConstrainedReal(_, _, bName) -> { A = aName; B = bName; Weight = freq }
                | ConstrainedVirtual(_, _, conn), _
                | _, ConstrainedVirtual(_, _, conn)
                | UnconstrainedVirtual(_, conn), _
